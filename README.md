...
- `cita fetch [--force] [--dry-run | --save] <selector>` — download the selected
  ...
  `--save` stores its metadata first. `--force` downloads again even when a
  cache file is present. `--dry-run` prints the URL without touching the local
  cache and cannot be combined with `--save`.
- `cita open [--force | --no-download | --browser] [--save] <selector>` — open
  the PDF in the system's default application. As with `fetch`, valid missing
  locators are resolved transiently unless `--save` is passed. By default,
  Cita uses the cached PDF or downloads it if needed. `--force` downloads a
  fresh copy, `--no-download` errors on a cache miss, and `--browser` opens the
  arXiv PDF URL without downloading or touching the local cache. `--save` is
  valid in every open mode.
