...
Four crates. `cita-core` is the light, provider-neutral vocabulary everything else
...
cita-core (light: serde, thiserror, async-trait, unicode-normalization)
   ↑                    ↑
cita-manifest        cita-inspire-client (+ reqwest, serde_json, url)
(+ toml, toml_edit,     ↑
   tempfile)            │
   ↑                    │
   └─────── cita ───────┘   (binary)
...
Library crates (`cita-core`, `cita-manifest`, `cita-inspire-client`) use
