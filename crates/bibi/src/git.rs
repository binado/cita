...
use bibi_manifest::{BIBLIOGRAPHY_FILE, Manifest, SourceSnapshot};
use std::{
...
pub fn commit(manifest_path: &Path) -> Result<()> {
    ...
    {
        println!("bibi references have no Git changes");
        return Ok(());
    }
    ...
    println!("Committed bibi references: {subject}");
    Ok(())
}
...
fn ensure_managed_files_unstaged(root: &Path, paths: &[&str]) -> Result<()> {
    ...
    match output.status.code() {
        Some(0) => Ok(()),
        Some(1) => bail!(
            "cita.toml or references.bib already has staged changes; commit or unstage the managed files before running `bibi commit`"
        ),
        _ => bail!(
...
fn rollback_error(
    ...
    match rollback_index(
        ...
        Err(cleanup) => anyhow::anyhow!(
            "{original:#}; additionally failed to restore bibi's index entries: {cleanup:#}"
        ),
    }
}
...
fn commit_message(
    ...
    let Some(old) = old else {
        return (
            "references: initialize bibi".into(),
            body_lines([], new, []),
        );
    };
    let added = new
