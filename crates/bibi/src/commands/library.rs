...
use bibi_manifest::{Library, LibraryError, MANIFEST_FILE};
use std::{
    fs,
    path::{Path, PathBuf},
};
...
/// the user to `bibi library list`. The hint also names the creating verb,
...
fn explain_lookup_failure(error: LibraryError, library: &Library) -> anyhow::Error {
    ...
    anyhow!("{error}; {known}; create it with `bibi library new {name}`")
}

pub(crate) fn init_library(cwd: &Path, path: Option<&Path>) -> Result<()> {
    let directory = resolve_from_caller(cwd, path);
    let existed = directory.join(bibi_manifest::LIBRARY_FILE).exists();
