...
fn bibi_without_git(cwd: &Path, args: &[&str]) -> Output {
    Command::new(env!("CARGO_BIN_EXE_bibi"))
        .current_dir(cwd)
        .args(args)
        .env("NO_COLOR", "1")
...
fn init_creates_schema_one_and_imports_an_existing_bibliography() {
    ...
    assert!(success(bibi(empty.path(), &["init"])).contains("Initialized"));
    assert_eq!(
...
fn init_defaults_to_the_current_directory_and_accepts_an_explicit_path() {
    ...
    let output = success(bibi(&nested, &["init"]));
...
fn init_rejects_a_library_root_without_creating_shelf_artifacts() {
    let directory = tempfile::tempdir().unwrap();
    success(bibi(directory.path(), &["library", "init"]));

    let error = failure(bibi(directory.path(), &["init"]));
    ...
    success(bibi(directory.path(), &["library", "list"]));
}

#[test]
fn library_and_shelf_initialization_cover_new_and_existing_projects() {
    let directory = tempfile::tempdir().unwrap();
    assert!(success(bibi(directory.path(), &["library", "init"])).contains("cita-library.toml"));
    assert!(success(bibi(directory.path(), &["library", "init"])).contains("Already initialized"));

    success(bibi(directory.path(), &["library", "new", "paper-one"]));
    assert!(directory.path().join("paper-one/cita.toml").is_file());

    let bibliography = directory.path().join("incoming/references.bib");
    fs::create_dir_all(bibliography.parent().unwrap()).unwrap();
    fs::write(
        &bibliography,
        format!("{}\n", entry("Imported", "Imported shelf", "")),
    )
    .unwrap();
    success(bibi(
        directory.path(),
    ...
    assert!(
        fs::read_to_string(directory.path().join("incoming/cita.toml"))
            .unwrap()
            .contains("[references.Imported]")
    );

    let adopted = directory.path().join("existing/project");
    fs::create_dir_all(&adopted).unwrap();
    success(bibi(&adopted, &["init"]));
    success(bibi(
        directory.path(),
        &[
            "library",
...
fn shelf_scoped_commands_are_isolated_and_import_paths_use_the_caller() {
    ...
    success(bibi(directory.path(), &["library", "init"]));
    for name in ["one", "two"] {
        success(bibi(directory.path(), &["library", "new", name]));
    }
    fs::write(
        directory.path().join("shared.bib"),
        format!(
            "{}\n",
            entry("SameKey", "Same identity", "doi={10.1000/same},")
        ),
    )
    .unwrap();
    for name in ["one", "two"] {
    ...
    assert!(success(bibi(directory.path(), &["list", "-s", "one"],)).contains("Same identity"));
    success(bibi(directory.path(), &["remove", "-s", "one", "SameKey"]));
    assert!(
    ...
    assert!(
        fs::read_to_string(directory.path().join("two/cita.toml"))
            .unwrap()
            .contains("SameKey")
    );

    fs::write(directory.path().join("two/references.bib"), "drift\n").unwrap();
    success(bibi(directory.path(), &["generate", "-s", "two"]));
    assert!(
        fs::read_to_string(directory.path().join("two/references.bib"))
            .unwrap()
            .contains("@misc{SameKey")
    );
    assert_eq!(
...
fn shelf_scoped_add_fetch_and_commit_use_the_shelf_context() {
    ...
    init_git(directory.path());
    success(bibi(directory.path(), &["library", "init"]));
    success(bibi(directory.path(), &["library", "new", "paper"]));
    ...
    handle.join().unwrap();
    assert!(
        fs::read_to_string(directory.path().join("paper/references.bib"))
            .unwrap()
            .contains("Fresh")
    );
    ...
    let fetched = success(bibi(
        directory.path(),
        &["fetch", "-s", "paper", "--cache-only", "Local"],
    ));
    assert!(fetched.contains("paper/.bibi/files/arxiv/2401.00042.pdf"));

    assert!(
        success(bibi(directory.path(), &["commit", "-s", "paper"],))
            .contains("references: initialize bibi")
    );
...
#[test]
fn library_batches_are_ordered_continue_after_failure_and_report_once() {
    let directory = tempfile::tempdir().unwrap();
    success(bibi(directory.path(), &["library", "init"]));
    ...
    let output = bibi(directory.path(), &["generate", "--all-shelves"]);
    assert!(!output.status.success());
    let stdout = String::from_utf8(output.stdout).unwrap();
    let stderr = String::from_utf8(output.stderr).unwrap();
    let alpha = stdout.find("Shelf alpha: generated").unwrap();
    ...
    assert!(!output.status.success());
    let stdout = String::from_utf8(output.stdout).unwrap();
    let alpha = stdout.find("Shelf alpha: already in sync").unwrap();
    let middle = stdout.find("Shelf middle: failed:").unwrap();
    let zeta = stdout.find("Shelf zeta: already in sync").unwrap();
    assert!(alpha < middle && middle < zeta, "{stdout}");
    success(bibi(directory.path(), &["generate", "-s", "middle"]));
    ...
    .unwrap();
    assert!(success(bibi(directory.path(), &["library", "list"])).contains("alpha"));
    let output = bibi(directory.path(), &["generate", "--all-shelves"]);
    let stdout = String::from_utf8(output.stdout).unwrap();
    assert!(!output.status.success());
    assert!(stdout.contains("Shelf alpha: failed:"), "{stdout}");
    assert!(stdout.contains("Shelf zeta: generated"), "{stdout}");
}
...
fn library_rejects_unknown_shelves_and_a_root_that_is_also_a_shelf() {
    let directory = tempfile::tempdir().unwrap();
    success(bibi(directory.path(), &["library", "init"]));
    let error = failure(bibi(directory.path(), &["list", "-s", "missing"]));
    ...
    // that does.
    assert!(
        error.contains("create it with `bibi library new missing`"),
        "{error}"
    );

    success(bibi(directory.path(), &["library", "new", "alpha"]));
    success(bibi(directory.path(), &["library", "new", "zeta"]));
    let error = failure(bibi(directory.path(), &["add", "-s", "alpah", "1207.7214"]));
    ...
    let error = failure(bibi(directory.path(), &["library", "list"]));
    assert!(error.contains("library root cannot contain"), "{error}");
}

#[cfg(unix)]
#[test]
fn failed_library_registration_leaves_a_valid_shelf_for_retry() {
    use std::os::unix::fs::PermissionsExt;

    let directory = tempfile::tempdir().unwrap();
    success(bibi(directory.path(), &["library", "init"]));
    fs::create_dir(directory.path().join("orphan")).unwrap();

    let original_mode = fs::metadata(directory.path()).unwrap().permissions().mode();
    let mut locked = fs::metadata(directory.path()).unwrap().permissions();
    locked.set_mode(0o555);
    fs::set_permissions(directory.path(), locked).unwrap();
    let output = bibi(directory.path(), &["library", "new", "orphan"]);
    let mut restored = fs::metadata(directory.path()).unwrap().permissions();
    restored.set_mode(original_mode);
    fs::set_permissions(directory.path(), restored).unwrap();

    assert!(!output.status.success());
    assert!(directory.path().join("orphan/cita.toml").is_file());
    assert!(directory.path().join("orphan/references.bib").is_file());
    assert!(
    ...
    success(bibi(directory.path(), &["library", "new", "orphan"]));
    assert!(
        fs::read_to_string(directory.path().join("cita-library.toml"))
            .unwrap()
...
#[test]
fn shelf_scope_rejects_a_shelf_missing_bibi_toml() {
    let directory = tempfile::tempdir().unwrap();
    success(bibi(directory.path(), &["library", "init"]));
    success(bibi(directory.path(), &["library", "new", "orphan"]));
    fs::remove_file(directory.path().join("orphan/cita.toml")).unwrap();

    let stderr = failure(bibi(directory.path(), &["list", "-s", "orphan"]));
...
fn init_ignores_a_malformed_git_directory() {
    ...
    success(bibi(directory.path(), &["init"]));
...
fn import_overwrite_rekeys_a_duplicate_and_leaves_others_untouched() {
    ...
    success(bibi(directory.path(), &["generate"]));
}
...
fn drift_blocks_reads_and_generate_repairs_output() {
    ...
    assert!(failure(bibi(directory.path(), &["list"])).contains("run `bibi generate`"));
    success(bibi(directory.path(), &["generate"]));
    assert!(success(bibi(directory.path(), &["list"])).contains("Alpha"));
}
...
fn commit_refuses_prestaged_managed_files_without_touching_the_index() {
    ...
    let error = failure(bibi(directory.path(), &["commit"]));
...
fn commit_help_mentions_the_managed_file_staging_guard() {
    let directory = tempfile::tempdir().unwrap();
    let help = success(bibi(directory.path(), &["commit", "--help"]));
    assert!(
...
fn commit_handles_a_head_that_does_not_contain_managed_paths() {
    ...
    assert!(success(bibi(directory.path(), &["commit"])).contains("references: initialize bibi"));
...
#[cfg(unix)]
#[test]
fn failed_initial_commit_removes_only_bibis_new_index_entries() {
    let directory = tempfile::tempdir().unwrap();
    init_git(directory.path());
    fs::write(directory.path().join("notes.txt"), "keep staged\n").unwrap();
    git_success(directory.path(), &["add", "notes.txt"]);
...
fn fetch_url_prints_only_the_url_without_touching_the_cache() {
    let directory = tempfile::tempdir().unwrap();
    arxiv_library(directory.path());
    fs::remove_dir_all(directory.path().join(".bibi")).unwrap();
    assert_eq!(
...
fn commit_reports_a_git_launch_failure_instead_of_guessing() {
    ...
    init_git(directory.path());
    success(bibi(directory.path(), &["init"]));
    let error = failure(bibi_without_git(directory.path(), &["commit"]));
...
fn init_does_not_require_an_available_git_executable() {
    let directory = tempfile::tempdir().unwrap();
    success(bibi_without_git(directory.path(), &["init"]));
    assert!(directory.path().join("cita.toml").is_file());
}
...
fn shelf_export_is_named_for_the_shelf_not_its_directory() {
    ...
    success(bibi(directory.path(), &["export", "-s", "paper"]));
...
fn library_export_is_ordered_continues_after_failure_and_reports_once() {
    ...
    let output = bibi(directory.path(), &["export", "--all-shelves"]);
    assert!(!output.status.success());
    let stdout = String::from_utf8(output.stdout).unwrap();
    let stderr = String::from_utf8(output.stderr).unwrap();
