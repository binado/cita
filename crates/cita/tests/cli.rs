...
fn batch_export_uses_an_existing_directory_and_continues_after_failures() {
    ...
    assert!(
        stdout.find("Shelf alpha: exported").unwrap() < stdout.find("Shelf main: failed").unwrap()
    );
    assert!(
        stdout.find("Shelf main: failed").unwrap() < stdout.find("Shelf zeta: exported").unwrap()
    );
