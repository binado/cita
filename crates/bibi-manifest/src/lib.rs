...
impl InspireEntry {
    fn project(&self) -> Result<Reference, ProjectionError> {
        bibi_inspire_client::project_inspire(
            &self.bibtex,
...
pub enum Error {
    ...
    #[error("generated bibliography drift at {path}; run `bibi generate`")]
    BibliographyDrift {
    ...
    #[error(transparent)]
    Bibtex(#[from] bibi_bibliography::Error),
...
impl Manifest {
    ...
    pub fn verify_bibliography(&self) -> Result<(), Error> {
        ...
        // A missing file is drift (repairable by `bibi generate`); any other
